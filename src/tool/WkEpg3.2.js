...
;(function (window) {
  if (!(typeof window['WkEpg'] === 'object' && window['WkEpg'] !== undefined))
    window['WkEpg'] = {}
  window['WkEpg'].AreaCreator = function (
    numrow,
    numcolum,
    directions,
    domsidstring,
    ...
    if (
      domsunfocusstyle !== undefined &&
      !(domsunfocusstyle.constructor === Array)
    )
      domsunfocusstyle = new Array(domsunfocusstyle)

    for (var i = 0; i < numrow * numcolum; i++) {
      doms[i] = window['WkEpg'].DomData(
        domsidstring + i,
        domsfocusstyle,
        domsunfocusstyle
      )
  ...
  window['WkEpg'].PageObj = function (curareaid, index, areas) {
    ...
    this.pageTurn = function (num, areaid) {
      if (this.areas !== undefined)
        this.areas[areaid === undefined ? this.curareaid : areaid].pageturn(
          num,
          areaid
        )
    }
    ...
    this.getfocusdom = function () {
      if (this.areas !== undefined)
        return this.areas[this.curareaid].doms[
          this.areas[this.curareaid].curindex
        ]
    }
    /**
     *
    ...
    this.move = function (dir) {
      ...
      if (dirindex === 0 || dirindex === 2) {
        ...
        if (tempcurarea.columnsort) {
          ...
          if (tempcurarea.circledir === 1)
            return tempcurarea.insidemove(true, dirindex, num, cross)
        } else {
          /*按行排序*/
        ...
        if (tempcurarea.columnsort) {
          ...
          if (!cross) {
            ...
            // cross = !cross
            // if ( num == 1 ) {
            //     if ((tempcurarea.curindex+1<tempcurarea.datanum)) {
            //         console.log(num)
            //         return tempcurarea.insidemove(false, dirindex, num);
            //     }
            //     if () {
            //         tempcurarea.pageturn(1)
            //     }
            // }
          }
          if (tempcurarea.circledir === 1)
      ...
      if (cross) {
        if (
          tempcurarea.endwiseCrossturnpage &&
          tempAreaEndTurnPageFlag &&
          (dirindex === 0 || dirindex === 2) &&
          ((dirindex === 0 && tempcurarea.curpage > 1) ||
            tempcurarea.cirturnpage ||
            (dirindex === 2 && tempcurarea.curpage < tempcurarea.pagecount))
        ) {
          ...
          tempcurarea.broadwiseCrossturnpage &&
          tempAreaEndTurnPageFlag &&
          (dirindex === 1 || dirindex === 3) &&
          ((dirindex === 1 && tempcurarea.curpage > 1) ||
            tempcurarea.cirturnpage ||
            (dirindex === 3 && tempcurarea.curpage < tempcurarea.pagecount))
        ) {
          if (!!tempcurarea.areaCorssEvent) {
            if (!tempcurarea.areaCorssEvent()) {
              return false
          ...
          if (tempcurarea.directions[dirindex] !== -1) {
            if (
              this.areas[tempcurarea.directions[dirindex]].lockin ||
              (this.areas[tempcurarea.directions[dirindex]].datanum !==
                undefined &&
                this.areas[tempcurarea.directions[dirindex]].datanum <= 0)
            )
              return -1
            ...
            if (
              tempcurarea.stablemoveindex !== undefined &&
              tempcurarea.stablemoveindex[dirindex] !== -1
            ) {
              var tmpmoveindex =
                tempcurarea.stablemoveindex[dirindex].split(',')
              if (isemptydom) {
                if (dirindex === 2) {
                  var lastcolumindex =
                    (tempcurarea.numrow - 1) * tempcurarea.numcolum
                  var tmpcolum = tempcurarea.curindex % tempcurarea.numcolum
                  stableindex = lastcolumindex + tmpcolum
                } else if (dirindex === 3) {
                  var tmprow = parseInt(
                    (tempcurarea.curindex + 1) / tempcurarea.numcolum
                  )
            ...
            if (!!this.areas[tmpstablearea].doms[tmpstable]) {
              ...
              /*如果不存在直接跳到下一个区域,使用当前区域的tempcurarea*/
              var nextAreaStation =
                tempcurarea.doms[tempcurarea.curindex].nextAreaStation
              if (!!nextAreaStation) {
                this.areas[parseInt(nextAreaStation.split('>')[0])].areain(
                  parseInt(nextAreaStation.split('>')[1]),
                  dirindex
                )
              }
            }
            ...
            /**/
            if (!!tempcurarea.crossEdgeEvent)
              tempcurarea.crossEdgeEvent(dirindex)
          }
        }
      } else {
        return tempcurarea.insidemove(false, dirindex, num)
  ...
  window['WkEpg'].DomData = function (
    ...
    return {
      // id: undefined,
      // nextAreaStation:undefined,
      // htmldom: undefined,
      // mylink:"",
      canfocus: true,
      htmldomid: tempdomid,
      tempfocusstyle: tempfocusstyles,
      tempunfocusstyle: tempunfocusstyles,
      focusstyle: tempfocusstyles,
      unfocusstyle: tempunfocusstyles,
      // focusEvent: undefined,
      // unfocusEvent:undefined,
      // domOkEvent:undefined,
      // area:undefined,
      ok: function () {
      ...
      changefocus: function (focusornot) {
        if (focusornot) {
          /*聚焦事件*/
  ...
  function Area(numrow, numcolum, doms, directions) {
    ...
    //当前页数 //this.endwiseCrossturnpage要打开
    this.curpage = 1
    //总页数
    this.pagecount = 1
    //锁
    ...
    this.staySource = [false, false, false, false]
    //保持方向来源
    ...
    /*新增翻页焦点控制锁*/
    this.pageTurnAndChangeFocusLock = false
  ...
  Area.prototype.setDarkfocus = function (
    darkPage,
    darkIndex,
    darkfocusstyle,
    darkunfocusstyle
    ...
    if (darkfocusstyle !== undefined && !(darkfocusstyle.constructor === Array))
      darkfocusstyle = new Array(darkfocusstyle)
    if (
      darkunfocusstyle !== undefined &&
      !(darkunfocusstyle.constructor === Array)
    )
      darkunfocusstyle = new Array(darkunfocusstyle)
  ...
  Area.prototype.changeDarkFocus = function () {
    for (var i = 0; i < this.doms.length; i++) {
      /*暗焦点*/
  ...
  //焦点翻页锁
  Area.prototype.pageTurnAndChangeFocus = function () {
    if (this.pageTurnAndChangeFocusLock) {
      return null
    }

    if (this.pageTurnFlag !== 0) {
      var num = this.pageTurnFlag
      if (this.endwiseCrossturnpage) {
        /*纵向翻页*/
        this.changefocus(this.curindex, false)
        if (num > 0) {
          if (this.datanum < (this.curindex % this.numcolum) + 1) {
            /*如果取数之后的数据总数比当前的焦点位置小*/
            /*聚焦到最后一个*/
          ...
          if (
            (this.datanum - 1) % this.numcolum <
            this.curindex % this.numcolum
          ) {
            /*如果上一页最后一行的位置比当前的焦点位置小*/
            /*聚焦到最后一个*/
            this.changefocus(this.datanum - 1, true)
          } else {
            /*聚焦到最后一行的当前位置*/
            this.changefocus(
              (Math.ceil(this.datanum / this.numcolum) - 1) * this.numcolum +
                (this.curindex % this.numcolum),
              true
            )
          }
        }
      } else if (this.broadwiseCrossturnpage) {
        /*横向翻页*/
        this.changefocus(this.curindex, false)
        /*当前行*/
  ...
  Area.prototype.changefocus = function (index, focusornot) {
    this.currow = parseInt(this.curindex / this.numcolum)
    this.curcolum = parseInt(this.curindex % this.numcolum)
    ...
    this.curindex = index
    this.currow = parseInt(index / this.numcolum)
    this.curcolum = parseInt(index % this.numcolum)
    if (!!this.doms[index]) {
  ...
  Area.prototype.setfocuscircle = function (circledir) {
    this.circledir = circledir
  }
  Area.prototype.insidemove = function (circle, dirindex, num, cross) {
    this.changefocus(this.curindex, false)
    var nextindex
    if (this.columnsort) {
      /*按列排序*/
      if (dirindex === 1 || dirindex === 3)
        nextindex = !circle
          ? this.curindex + num * this.numrow
          : num > 0
          ? cross
            ? this.curindex - (this.numcolum - 1) * this.numrow
            : this.curindex + num * this.numrow
          : cross
          ? this.curindex + (this.numcolum - 1) * this.numrow
          : this.curindex + num * this.numrow
      ...
      if (dirindex === 0 || dirindex === 2)
        nextindex = !circle
          ? this.curindex + num * this.numcolum
          : num > 0
          ? cross
            ? this.curindex - (this.numrow - 1) * this.numcolum
            : this.curindex + num * this.numcolum
          : cross
          ? this.curindex + (this.numrow - 1) * this.numcolum
          : this.curindex + num * this.numcolum
  ...
  Area.prototype.changtop = function () {}
  Area.prototype.getid = function () {
    return this.id
  }
  Area.prototype.clearfocus = function () {
    this.changefocus(this.curindex, false)
    this.curindex = 0
  }
  Area.prototype.setendwiseCrossturnpage = function (tempvalue) {
    this.endwiseCrossturnpage = tempvalue
  }
  Area.prototype.setbroadwiseCrossturnpage = function (tempvalue) {
  ...
  Area.prototype.ok = function () {
    ...
    if (!this.areaOkAfterEventLock && this.areaOkAfterEvent !== undefined)
      this.areaOkAfterEvent()
    this.doms[this.curindex].ok()
  }
  Area.prototype.areain = function (stableindex, dir) {
    if (this.staySource && this.staySource[dir]) {
      var outdir = dir < 2 ? dir + 2 : dir - 2,
        oldareaid = this.pageobj.curareaid
      this.directions[outdir] = oldareaid
    }
    if (!!this.areaInBeforeEvent) {
      if (this.areaInBeforeEvent(dir)) {
        return true
      }
    }
    this.pageobj.curareaid = this.id
  ...
  Area.prototype.areaout = function (stableindex, dir) {
    if (!!this.areaOutBeforeEvent) {
      if (this.areaOutBeforeEvent(dir)) {
        return true
      }
    }
    if (this.staydirarray[dir]) {
      if (this.stayindexarray[dir] !== undefined)
        this.stayindexarray[dir] = stableindex
      if (this.staystylearray[dir] !== undefined) {
        ...
        this.doms[this.stayindexarray[dir]].changestyle(
          this.staystylearray[dir]
        )
  ...
  Area.prototype.pageturn = function (num, areaid) {
    if (this.lock) return
    if (this.curpage !== undefined && this.pagecount !== undefined) {
      ...
      if (num > 0 || this.focusfirst) {
        ...
        nextindexid =
          this.datanum === undefined ? this.doms.length - 1 : this.datanum - 1
      }
      if (
        this.darkfocusstyle !== undefined &&
        this.darkunfocusstyle !== undefined
...
;(function (window) {
  ...
  window['WkEpg'].Player = {
    ...
    initMedia: function (_left, _top, _width, _heigth) {
      ...
      var InstanceState = window.mp.bindNativePlayerInstance(instanceId)
      var playListFlag = 0
    ...
    playVodByTime: function (url, playType, beginTime, playSecond) {
      var mediaStr = '[{mediaUrl:"' + url + '",'
      mediaStr += 'mediaCode: "jsoncode1",'
      mediaStr += 'IsHD:"0",'
      mediaStr += 'mediaType:2,'
    ...
    setPlayComplete: _playCompleteTool(function () {
    ...
    /* setMute getMuteFlag  getVolume*/
    vodRelease: function () {
      window.mp.releaseMediaPlayer(window.mp.getNativePlayerInstanceID())
    },

    setMute: function () {
      if (window.mp.getMuteFlag() == 1) window.mp.setMuteFlag(0)
      else window.mp.setMuteFlag(1)

      if (!!this.onMuteState) this.onMuteState()
    },
    getMuteFlag: function () {
      return window.mp.getMuteFlag()
    },
    getVolume: function () {
      return window.mp.getVolume()
    },
    volUp: function () {
      if (window.mp.getMuteFlag() === 1) window.mp.setMuteFlag(0)
      var curVolume = parseInt(window.mp.getVolume())
      curVolume += 5
      if (curVolume >= 100) curVolume = 100
      window.mp.setVolume(curVolume)
      if (!!this.onVolume) this.onVolume(curVolume)
    },
    volDown: function () {
      var curVolume = parseInt(window.mp.getVolume())
      curVolume -= 5
      if (curVolume <= 0) curVolume = 0
      if (curVolume === 0) window.mp.setMuteFlag(1)
      window.mp.setVolume(curVolume)
      if (!!this.onVolume) this.onVolume(curVolume)
    }
  }

  function _playCompleteTool(fn) {
    var t
    return function () {
      var args = arguments,
        m = this
...
;(function (window) {
  ...
  window['WkEpg'].Util = {
    ...
    setCookie: function (key, val, days) {
      ...
      document.cookie =
        key + '=' + escape(val) + ';expires=' + exp.toGMTString()
    },
    ...
    cookie: function (variantName, value, days) {
      try {
        if (value) {
          this.setCookie(variantName, value, days)
        } else {
          return this.getCookie(variantName)
        }
      } catch (e) {}
    },
    ...
    getUrlParam: function (strname, url) {
      ...
      if (typeof url != 'undefined') hrefstr = url
      pos = hrefstr.indexOf('?')
      /*没有参数，则直接跳出*/
      if (pos == -1 && hrefstr.indexOf('=') == -1) return null
    ...
    getUrlParameterObj: function (url) {
      ...
      var obj = null
      /*没有参数，则直接跳出*/
      if (pos == -1 && url.indexOf('=') == -1) return obj
    ...
    log: function (error) {
      ...
      if (self.maindiv === null) {
        ...
        self.e.style.cssText =
          'z-index:9999; font-size: 12px; position:absolute; top:10px; left:10px; width:1240px; height:auto; overflow:hidden; padding:10px; background:#14affa; color: red;word-break:break-all; display:block; filter:alpha(opacity=0.8);-moz-opacity: 0.8;-khtml-opacity: 0.8;opacity: 0.8;'
      }
  ...
  window['WkEpg'].Nav = {
    ...
    keyDo: function (keyCode) {
      ...
      if (typeof DataAccess == 'undefined') {
        // document.write('<script type="text/javascript" src="/NewFrameWork/newWeb/js/stb.js"></script>');
        KEY_DOWN = 40
        KEY_UP = 38
        KEY_LEFT = 37
        KEY_RIGHT = 39
        ...
        case KEY_IPTV_EVENT:
          ...
          switch (typeStr) {
            ...
            case 'EVENT_MEDIA_END':
              ...
              if (!!window['WkEpg'].Player) {
                window['WkEpg'].Player.onplayComplete &&
                  window['WkEpg'].Player.onplayComplete()
              }
              return
    ...
    keyVolUpEvent: function () {
      if (!!window['WkEpg'].Player && window['WkEpg'].Player.volUp) {
        window['WkEpg'].Player.volUp()
      }
    },
    keyVolDownEvent: function () {
      if (!!window['WkEpg'].Player && window['WkEpg'].Player.volDown) {
        window['WkEpg'].Player.volDown()
      }
    },
    keyMuteEvent: function () {
      if (!!window['WkEpg'].Player && window['WkEpg'].Player.setMute) {
        window['WkEpg'].Player.setMute()
      }
    },
    ...
    keyPortalEvent: function () {
      if (!PX_TQMY_EPG_INFO.HOST) return
      window.location.href = GDEPGPage(PUB_PAGE['main'])
    },
    keyMenuEvent: function () {
      if (!PX_TQMY_EPG_INFO.HOST) return
      window.location.href = GDEPGPage(PUB_PAGE['main'])
    },
